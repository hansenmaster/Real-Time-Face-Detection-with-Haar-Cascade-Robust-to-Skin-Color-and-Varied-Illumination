				...
				for img in os.listdir('dataset/'+name+'/'+lux):
					...
					for x,y,w,h in faces:
						...
						if (indikatorPos==1): #predicted not face HV in NN
							cv2.rectangle(bebas, (int(x/scale), int(y/scale)), (int((x+w)/scale), int((y+h)/scale)), (0, 255, 0), 6)
					count += 1
					cv2.imwrite("eval/"+name+"/"+lux+"post/%d.jpg"% count,bebas)
FaceDetectionPost()